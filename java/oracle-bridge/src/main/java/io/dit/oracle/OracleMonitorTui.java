...
/**
 * Interactive terminal dashboard for Oracle monitoring using Lanterna Screen layer.
 * Layout (~160x45):
 *   Row 0:      Title bar
 *   Row 1-8:    Load Profile (left) + Top Waits (right)
 *   Row 9-28:   Sessions table
 *   Row 29-42:  SQL Detail panel
 *   Row 43-44:  Footer / status bar
 */
public final class OracleMonitorTui {
    ...
    private final WaitEventDeltaTracker waitDeltaTracker = new WaitEventDeltaTracker();
    ...
    // Cached data - guarded by dataLock
    ...
    public OracleMonitorTui(Map<String, String> options) {
        this.host = req(options, "host");
        this.port = req(options, "port");
        this.serviceName = req(options, "service-name");
        this.user = req(options, "user");
        this.password = req(options, "password");
        this.connectTimeoutSec = intVal(options.get("tcp-connect-timeout-seconds"), 5);
        this.readTimeoutMs = intVal(options.get("call-timeout-ms"), 5000);
    ...
    public void run() throws IOException {
        ...
        try {
            ...
            while (running.get()) {
                ...
                // Collect at configured interval
                ...
                // Only render when data changes or user input
                if (needsRender) {
                ...
                // Poll loop ~100ms (responsive input, no flicker)
    ...
    private void collectData(Connection conn) throws SQLException {
        ...
        Map<String, Object> data = OracleCollector.collectAll(conn);

        // Replace V$EVENTMETRIC (60s lag) with real-time V$SYSTEM_EVENT delta
        try {
        ...
        Map<String, Object> metrics = OracleCollector.mapMetrics(
            sysmetric != null ? sysmetric : new java.util.LinkedHashMap<String, Object>(),
            sysstat != null ? sysstat : new java.util.LinkedHashMap<String, Object>()
        );
        ...
        // Push to ring buffer for sparklines
    ...
    private void render(Screen screen, TerminalSize size) {
        ...
        String instanceName = inst != null ? str(inst.get("instance_name")) : "Oracle";
        String hostName = inst != null ? str(inst.get("host_name")) : host;
        String version = inst != null ? str(inst.get("version")) : "";
        String title = String.format(" DIT | %s@%s | %s | Collected: %s ",
            instanceName, hostName, version, lastCollectTime);
        drawBar(screen, row, w, title, TITLE_FG, TITLE_BG);
        row++;

        // ── Load Profile Panel + Top Waits Panel (side by side) ──
        int leftW = w / 2;
        int rightW = w - leftW;
        ...
        drawBox(screen, row, 0, w, sqlPanelH, "Top SQL (10 min)", BORDER_FG);
    ...
    private void handleKey(KeyStroke key, Screen screen) throws IOException {
        if (key.getKeyType() == KeyType.Character) {
            ...
            if (c == 'q' || c == 'Q') {
                ...
                // Force refresh on next cycle (already continuous)
            }
            ...
            sessionSelect = Integer.MAX_VALUE; // will be clamped in render
        } else if (key.getKeyType() == KeyType.Tab) {
            // Toggle focus between sessions and SQL (scroll SQL)
    ...
    private Connection openConnection() throws SQLException {
        ...
        props.setProperty("oracle.net.CONNECT_TIMEOUT", Integer.toString(connectTimeoutSec * 1000));
        props.setProperty("oracle.jdbc.ReadTimeout", Integer.toString(readTimeoutMs));
        props.setProperty("v$session.program", "dit-bridge");
    ...
    private static String req(Map<String, String> opts, String key) {
